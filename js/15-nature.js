 ...
 * ==================================================================== */
/* ---- 雲の3種(高度帯で粒度を変える)---------------------------------
 * 【なぜ従来の雲では光条が出なかったか — 実測(ヴァンセンヌ 夕 zoom0.25)】
 *   太陽アンカーの仰角     … 7.4 度(SUN_ANCHOR_ELEV_MAX = 0.13rad で頭打ち)
 *   遮蔽グローの見かけ半径 … 24.6 度(17-postfx.js rayOccGlowAngle 0.86rad)
 *   従来の雲(34枚)の見かけ半径 … 4.8〜28.6 度 / 仰角 6.6〜20 度
 * つまり **雲1枚がグローより大きい**。しかも遮蔽パスは「その画素から太陽
 * までの線分」を積分するので、**太陽のすぐ脇にある雲が扇の形を決めてしまう**。
 * 隙間なく塗る大雲が1枚そこに来た時点で、外側に小さい雲を何枚足しても
 * プロファイルは変わらない(中サイズを 8→16 枚に増やして円周方向の輝度比を
 * 測ったが、ほぼ無変化だった)。
 * 天使の梯子は「遮蔽物が光源を **部分的に** 隠す」ことでしか生まれないので、
 * 必要なのは「太陽の高度帯に、**グローより小さい雲が隙間を空けて並ぶ**」状態。
 *
 * ※ 1枚のテクスチャに穴を開けた「ちぎれ雲」も試したが、拡大されると
 *   白い玉が一列に並んだ人工的な絵になり(空としての自然さを壊す)、
 *   輝度比も改善しなかったので採らなかった。隙間は **雲と雲の間** で作る。
 *
 * 【採った構成】
 *   H 高い塊雲 12枚 仰角 12〜20 度。従来と同じ大きさ・同じテクスチャ。
 *                   太陽より上なので扇を潰さず、空の見た目は従来のまま。
 *   B 雲の列   24枚 仰角 4〜10 度 = 太陽の高度帯。見かけ半径 3〜5 度の雲を
 *                   3枚ひと組にし、11〜16 度おきに並べて 4〜6 度の隙間を
 *                   空ける(8群)。この隙間がそのまま光条の通り道になる。
 *   S 小さい雲 12枚 仰角 3〜10 度。見かけ半径 1.5〜3 度の粒。列の隙間を
 *                   さらに細かく刻む。
 * 合計 48 枚(従来 34 枚)。晴で見えるのは 38 枚(従来 16 枚)。遮蔽パスが
 * 実際に描くのは「画面内の雲 + 3」で、5城の夕で実測 12〜17 枚(従来 6〜10)。 */
var NAT_CLOUD_TIERS = (function(){
  var defs = reducedMotion
    ? [{ key:'H', n:6 }, { key:'B', n:12 }, { key:'S', n:6 }]
    : [{ key:'H', n:12 }, { key:'B', n:24 }, { key:'S', n:12 }];
  var arr = [], t, j;
  for (t=0;t<defs.length;t++){
    for (j=0;j<defs[t].n;j++) arr.push({ key: defs[t].key, k: j, n: defs[t].n });
  }
  return arr;
})();
var NAT_CLOUD_MAX = NAT_CLOUD_TIERS.length;
var natClouds = [];
...
function natLayoutClouds(){
  ...
  var A = ZMAX * 0.78;
  // 低い2種の高度に足す下駄。カメラ高さ(ZMAX 600 の城・zoom0.25 で実測 66m)
  // の代わり。これが無いと「雲までの距離 × 仰角」が見かけの仰角にならない。
  var camB = ZMAX * 0.11;
  // B(雲の列)の群れごとに使い回す値。群れの先頭(j===0)で引き直す。
  var gA = 0, gR = R, gElev = 0.1, gStep = 0.2, gRank = 0;
  var i;
  for (i=0;i<natClouds.length;i++){
    var c = natClouds[i], tier = NAT_CLOUD_TIERS[i];
    var u = rnd();
    /* 高度は「その雲までの距離 × 仰角」で決める。ZMAX だけから決めると、
     * 城が大きいほど雲の仰角が上がって画面上端の外へ出てしまう(ヴァンセ
     * ンヌで実測: 仰角 11-20 度 = 見える空の帯 0-11 度の外)。
     * 方位は黄金角で刻む -- 黄金角列は **どの長さの先頭部分を取っても全周へ
     * 均等に散る**(低食い違い列)ので、雲量が減って前半しか出ない晴天でも
     * 空の一角へ固まらない。種をまたいだ通し番号ではなく種の中の番号 k で
     * 刻む(そうしないと種ごとに見たときに偏る)。 */
    if (tier.key === 'H'){
      c.a = (tier.k * 2.399963 + rnd()*0.5) % (Math.PI*2);
      c.r = R * (0.55 + 0.55*rnd());
      c.alt = A * 0.38 + c.r * (0.10 + 0.05*u);   // 仰角 約12〜20度(太陽の上)
      c.w = ZMAX * (0.80 + 0.55*rnd());           // 従来の大きい側そのまま
      c.rank = (tier.k + 0.5) / tier.n;
    } else if (tier.key === 'B'){
      /* ★太陽の高度帯には「3枚ひと組の群れ」を置く。
       * 実測で分かったのは、雲を小さく散らすだけでは駄目だということ:
       *   ・グローより大きい雲が1枚 → 扇が丸ごと潰れて帯1本(従来の絵)
       *   ・小さい雲をばらまく       → どの半径線も遮り切れず、太陽まわりが
       *                                一様に 1.5 倍明るいだけの霞になる
       * 光条として読めるのは「**遮り切った暗い楔** と **抜けた明るい隙間** が
       * 交互に並ぶ」ときだけ。そこで、見かけ半径 3〜5 度の雲を 11〜16 度
       * 間隔で3枚並べ、間に 4〜6 度の隙間を意図的に空ける。群れの幅は
       * 30 度前後で、8群あるので全周の 4分の3 が埋まる = 太陽がどの方位に
       * 来ても、たいてい群れのどこかに重なる。 */
      var gSize = 3;
      var g = Math.floor(tier.k / gSize), j = tier.k % gSize, gN = tier.n / gSize;
      if (j === 0){
        gA = (g * 2.399963 + rnd()*0.30) % (Math.PI*2);
        gR = R * (0.60 + 0.45*rnd());
        gElev = 0.075 + 0.100*rnd();              // 仰角 約4〜10度(太陽の帯)
        gStep = 0.19 + 0.09*rnd();                // 隣との間隔(rad)
        // 群れは丸ごと出るか丸ごと消えるかにする(半分だけ出ると隙間の
        // 並びが崩れる)。ここを詰めておかないと、雲量 0.46 の晴で群れの
        // 大半が半透明になり、遮り切れずに霞だけが残る。
        gRank = (g + 0.5) / gN * 0.38;
      }
      c.a = (gA + (j - (gSize-1)*0.5) * gStep + (rnd()-0.5)*0.02) % (Math.PI*2);
      c.r = gR * (1 + (rnd()-0.5)*0.06);
      c.alt = camB + c.r * (gElev + (rnd()-0.5)*0.036);   // 群れの中で高さを散らす
      c.w = ZMAX * (0.24 + 0.14*rnd());           // 見かけ半径 3〜5度
      c.rank = gRank;
    } else {
      c.a = (tier.k * 2.399963 + rnd()*0.5) % (Math.PI*2);
      c.r = R * (0.55 + 0.55*rnd());
      c.alt = camB + c.r * (0.055 + 0.115*u);     // 仰角 約3〜10度
      c.w = ZMAX * (0.11 + 0.13*rnd());           // 見かけ半径 1.5〜3度
      c.rank = (tier.k + 0.5) / tier.n * 0.60;
    }
