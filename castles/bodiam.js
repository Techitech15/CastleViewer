...
function buildBodiam(){
  ...
  /* ================================================================ *
   * 手続き的テクスチャ -- 共有工房 CastleTex を使う
   * ================================================================ *
   * 以前はこのファイル内に約700行の工房(buildBodiamTextureAtelier)を
   * 抱えていたが、他の4城でも同じものを使うため js/02-texture.js へ
   * 切り出した。アルゴリズム・シード・定数は当時のままで、工房の既定値
   * がそのままボディアムの値になっている。したがってここでは何一つ
   * 上書きしない -- 上書きした瞬間にボディアムの見た目が変わる。
   *
   * 使える kind、texMat の opt、そして「色 / タイルサイズ / 法線強度」を
   * 城ごとに変える方法は js/02-texture.js の冒頭コメントに書いてある。
   *
   * ここでローカルの薄いラッパ(TEX / texMat / applyWorldUVs)を置いて
   * いるのは、この下に約2500行ある呼び出し側を一切書き換えずに済ませる
   * ため。呼び出し方は切り出し前とまったく同じ。 */
  var TEXKIT        = CastleTex.kit({ id: 'bodiam' });
